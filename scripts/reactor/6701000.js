...
function act() {
    if (!rm.haveItem(4001268)) {
        var rand = (Math.random() * 10) + 1;
      //  var randMap = (Math.random() * 9) + 1;
        if (rand  > 0) {
            if (rm.canHold(4001268)) {
                rm.gainItem(4001268, 1);
                rm.gainItem(4032381, -1);
                rm.gainItem(1142767,1,15,15,15,15);
                rm.warp(910000000);
                rm.worldMessage(5, "[跑酷初賽1] " + " 玩家" + rm.getChar().getName() + " 從跑酷初賽1脫穎而出並獲得了跑酷活動紀念勳章!!");
