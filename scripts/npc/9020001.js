...
function start() {
   status=-1;
   mapId=cm.getMapId();
   if (mapId==103000800) curMap=1;
   else if (mapId==103000801) curMap=2;
   else if (mapId==103000802) curMap=3;
   else if (mapId==103000803) curMap=4;
   else if (mapId==103000804) curMap=5;
...
function action(mode, type, selection) {
   ...
   if (curMap==1) {
      ...
      if (playerStatus) {
         ...
         if (status==0 && preamble==null) {
            ...
            if (complete !=null) {
               ...
               if(eim.getProperty("getTicket_"+cm.getChar().getId()) == "1"){ //已取得通行證的玩家無法再次取得
                  cm.sendOk("請將#v4001008# #z4001008#交給隊長哦");
                  cm.dispose(); return;
               }
               ...
               if ( !qcorr) {
                  ...
                  if (qcorr) {
                     ...
                     eim.setProperty("getTicket_"+cm.getChar().getId(), "1");
      ...
      if (stage5done==null) {
         ...
         // Give rewards and warp to bonus
         if (status==0) {
            cm.sendNext("真的很不可思議！");
         }
      ...
      if (status==0) {
         ...
         if (preamble==null) {
            ...
            if (complete !=null) {
               ...
               // Check for people on ropes and their positions
               var totplayers=0;
...
function getPrize(eim, cm) {
   ...
   if (itemSetSel < 0.3) itemSet=prizeIdScroll;
   else if (itemSetSel < 0.6) itemSet=prizeIdEquip;
