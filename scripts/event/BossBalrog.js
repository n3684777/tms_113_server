...
var 遠征怪物名稱     = "魔王巴洛谷";
var boss             = "BossBalrog";
var bossMapId        = 105100300;
var endMapId         = 105100100;
var 獎勵地圖         = 105100301;
var map              = null;
var endMap           = null;
...
var rewards = [
		...
		[ // 第3名奖励
			{ itemId: 4000005, quantity: 5 },
			{ itemId: 4000005, quantity: 3 },
		]
...
function monsterValue(eim, mobId) { // 每當有怪物被消滅
    switch (mobId) {
        case 8830007:
            eim.setProperty("killed_8830007", "true");
            killHt++;
            break;
        case 8830001:
            eim.setProperty("killed_8830001", "true");
            killHt++;
            break;
        case 8830002:
            eim.setProperty("killed_8830002", "true");
            killHt++;
            break;
        default:
            return 1; // 非目標怪物，直接返回
    ...
    // 條件 1：確認是否所有目標怪物都已被擊敗
    var allKilled = (
        eim.getProperty("killed_8830007") === "true" &&
        eim.getProperty("killed_8830001") === "true" &&
        eim.getProperty("killed_8830002") === "true"
    );

    // 條件 2：killHt 記錄是否達到 3
    if (allKilled || killHt >= 3) {
        clearPQ(eim);
    }
...
function monsterDamaged(eim, player, mobId, damage){
	...
	if(mobId != 8830007 && mobId != 8830001 && mobId != 8830002 ) return;
	if(damage < 0){
		damage = 2147483000;
	}
...
function adjust(eim) {
	if(map == null){
		...
		map.resetFully();
		//map.spawnNpc(1061014, new java.awt.Point(-5, -86));
		...
		eim.setProperty("killed_8830007", "false"); // 初始化擊殺紀錄
        eim.setProperty("killed_8830001", "false");
        eim.setProperty("killed_8830002", "false");
		killHt = 0;
	}
}
...
function end(eim){
   init();
   openhappy = eim.getProperty("open");
   if(openhappy == 1){
	   eim.disposeIfPlayerBelow(99, 獎勵地圖);
   }else{
