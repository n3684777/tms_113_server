...
var 左側頭顱血量 = 450000000;
var 右側頭顱血量 = 450000000;
var A頭血量 = 990000000;
var B頭血量 = 980000000;
var C頭血量 = 990000000;
var 左手血量 = 990000000;
...
function setup(partyid){
    if (map_1 == null) {
        var instanceName = boss + partyid;
        var eim = em.newInstance(instanceName);
        map_1 = eim.createInstanceMap(hole1);
        map_2 = eim.createInstanceMap(hole2);
        map_3 = eim.createInstanceMap(hole3);
        exitMap = em.getChannelServer().getMapFactory().getMap(endMapId);
        eim.setProperty("partyid", String(partyid));
        eim.setProperty("stage", "1");
        eim.setProperty("stage1", "false");
        eim.setProperty("stage2", "false");
        eim.setProperty(boss, "true");
        eim.setProperty("左側頭顱血量", String(左側頭顱血量));
        eim.setProperty("右側頭顱血量", String(右側頭顱血量));
        eim.setProperty("A頭血量", String(A頭血量));
        eim.setProperty("B頭血量", String(B頭血量));
        eim.setProperty("C頭血量", String(C頭血量));
        eim.setProperty("左手血量", String(左手血量));
        eim.setProperty("右手血量", String(右手血量));
        eim.setProperty("翅膀血量", String(翅膀血量));
        eim.setProperty("腿血量", String(腿血量));
        eim.setProperty("尾巴血量", String(尾巴血量));
        eim.setProperty("本體血量", String(本體血量));
        eim.startEventTimer(time); //時間
    }
    return eim;
}
...
function monsterValue(eim, mobId) {
   switch(mobId){
      case 8810000: //打死左頭顱 stage 設 2
         eim.setProperty("stage", "2");
         break;
      case 8810001: //打死右頭顱 stage 設 3
...
function playerEntry(eim, player, currentDate){
   ...
   switch(stage){
      case "1":
         map = map_1;
         if (eim.getProperty("stage1").equals("false")) {
            map.spawnSpecialHorntail(959, 230, partyid, parseInt(stage));
            eim.setProperty("stage1", "true");
         }
         break;
   }
   killHt = 0;
...
function clearPQ(eim) {
    ...
    for (var i = 0; i < playerList.size(); i++) {
        ...
        if (playerBossDamage == 0 && 玩家紀錄判斷 == null) {
            playerID.setCharInfoNum(遠征排名計算, String(playerDamage));
...
function changedMap(eim, player, mapid){
    ...
    switch(stage){
        case "2":
            if (eim.getProperty("stage2").equals("false")) {
                eim.getMapInstance(1).spawnSpecialHorntail(-317, 230, partyid, parseInt(stage));
                eim.setProperty("stage2", "true");
            }
            break;
        case "3":
            //eim.getMapInstance(2).spawnSpecialHorntail(71, 260, partyid, parseInt(stage));
            break;
    }
}
