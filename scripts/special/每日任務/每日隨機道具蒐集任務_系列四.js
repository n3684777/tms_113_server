...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    if (cm.getPlayer().getPrizeLog("" + Year + "年" + MonthS[Month] + tzc + "日" + log_2 + "") <= 0 || name.getCharInfoNum("本日蒐集任務接取狀態_3") == 0) {
      var randomItems = [];
      var randomRewards = [];
      var chosenIndexes = [];
      ...
      name.setCharInfoNum("本日隨機蒐集物_3", JSON.stringify(randomItems));
      name.setCharInfoNum("本日蒐集任務接取狀態_3", 1);
      ...
      name.setCharInfoNum("本日隨機蒐集物獎勵_3", JSON.stringify(randomRewards));
      cm.getPlayer().setPrizeLog("" + Year + "年" + MonthS[Month] + tzc + "日" + log_2 + "");
    } else {
      try {
        var record = cm.getQuestRecord(95046); //本日隨機蒐集物
        randomItems = JSON.parse(record.getCustomData());
        var record_2 = cm.getQuestRecord(95047); //本日隨機蒐集物獎勵
    ...
    if (!allItemsExist) {
      ...
      cm.dispose();
		...
		name.setCharInfoNum("本日蒐集任務接取狀態_3", 0);
