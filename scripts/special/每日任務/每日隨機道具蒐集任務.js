...
var numItemsToCollect = 2; //隨機的道具量
var numRewardsToGive = 2; //隨機的獎勵數量
var 本日可接取次數 = 5;

//設定隨機抽取的道具，請不要小於隨機抽取數量(numItemsToCollect)
var needItem = [
  //道具代碼 最小數量 最大數量
  [4007000, 1000, 2000],
  [4007001, 600, 2000],
...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    if (cm.getPlayer().getPrizeLog("" + Year + "年" + MonthS[Month] + tzc + "日" + log_2 + "") <= 0 || name.getCharInfoNum("本日蒐集任務接取狀態") == 0) {
      var randomItems = [];
      var randomRewards = [];
      var chosenIndexes = [];
      ...
      name.setCharInfoNum("本日隨機蒐集物", JSON.stringify(randomItems));
      name.setCharInfoNum("本日蒐集任務接取狀態", 1);
      ...
      name.setCharInfoNum("本日隨機蒐集物獎勵", JSON.stringify(randomRewards));
      cm.getPlayer().setPrizeLog("" + Year + "年" + MonthS[Month] + tzc + "日" + log_2 + "");
    } else {
      try {
        var record = cm.getQuestRecord(95040); //本日隨機蒐集物
        randomItems = JSON.parse(record.getCustomData());
        var record_2 = cm.getQuestRecord(95041); //本日隨機獎勵物
    ...
    var record = cm.getQuestRecord(95040); //本日隨機蒐集物
    randomItems = JSON.parse(record.getCustomData());
    var record_2 = cm.getQuestRecord(95041); //本日隨機蒐集物
    randomRewards = JSON.parse(record_2.getCustomData());
    for (var i = 0; i < randomItems.length; i++) {
    ...
    if (!allItemsExist) {
      ...
      cm.dispose();
		...
		name.setCharInfoNum("本日蒐集任務接取狀態", 0);
