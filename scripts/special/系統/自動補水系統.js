...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		if (selection >= 128) {
			...
			PercentageRun = true;
			cm.sendGetNumber("請輸入低於百分比多少進行補品", 1, 1, 100);
		} else {
			var ii = MapleItemInformationProvider.getInstance();
			text = "#e請選擇補品:#n\r\n\r\n#b";
			...
			while (itemList.hasNext()) {
				...
				var itemEffect = ii.getItemEffect(item.getItemId());
				if (itemEffect != null) {
					if (itemEffect.getHp() == 0 && itemEffect.getHpR() == 0 && SelectPotion)
						continue;
					if (itemEffect.getMp() == 0 && itemEffect.getMpR() == 0 && !SelectPotion)
						continue;
				}
