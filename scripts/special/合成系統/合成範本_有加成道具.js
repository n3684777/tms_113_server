...
var 上頁腳本 = "合成系統";
var 腳本路徑 = "合成系統/合成範本_有加成道具";
var points;
var 失敗不扣除第一樣道具 = 0;  // 0 = 關，會扣除。 1 = 開，不會扣除
var 隨機屬性 = 0; // 0 = 關 1 = 開

//*****************
var 成功率增加道具 = 4000036;//設定0就是不要
var 成功機率 = 10;
//*****************
var 絕對成功道具 = 4000037;//設定0就是不要
var 是否上廣 = 0; // 0 = 不上廣 1 = 上廣
var items = [
//[獲得物品ID, 獲得物品數量, 等級, 機率, 四屬, 魔物攻, 保底額度 999 = 無保底, 存取資料代號(代號請勿重複，不同腳本也不能重複)
	//道具一基本設定
	[1122025 , 1, 10, 100, 0, 10, 10, 200490],
	[1122030 , 1, 10, 100, 0, 0, 10, 200491],
	[1122035 , 1, 10, 100, 0, 0, 10, 200492],
]; 
    ...
    if (status == 0) {
        ...
        if (hasRequiredItems(cm.getPlayer(), requiredItems)) {
            if(ItemRandom <= 設定機率 || points >= 保底次數){
                ...
                if(四屬數值 > 0 || 魔物攻值 > 0){
					var _EQU = cm.getEquip(items[select][0]);
					   _EQU.setStr(四屬數值);  
					   _EQU.setDex(四屬數值);
					   _EQU.setInt(四屬數值);
					   _EQU.setLuk(四屬數值);
					...
					cm.GAMEPLAYERLOG(name, "++獲得道具", items[select][0], items[select][1]);					
				}else{
					if(隨機屬性 == 1){
						var item = MapleInventoryManipulator.addbyId_Gachapon(cm.getPlayer().getClient(), items[select][0], 1);
						if(是否上廣 == 1){
						...
						if(是否上廣 == 1) {
							cm.showSmegaItem("被他成功合成了，大家恭喜他吧！", items[select][0]);
						}
					}
