...
var UP_item = [
   ...
   //兌換物代碼填寫0則不會計算
	...
	[
		[4000006, 1, 4000009, 1, 4000011, 1, 4000012, 1, 0, 1, 0, 1],
		[4000006, 1, 4000009, 1, 4000011, 1, 4000012, 1, 0, 1, 0, 1],
	],
]
        ...
        var msg = "";
            ...
            for (var i = 0; i < 96; i++) {
               ...
               itemId = cm.getInventory(1).getItem(i).getItemId();
                  ...
                  if (Eq_item.some(function(item) { return item[0] === itemId; })) {
                    var select = -1;  // 初始化為 -1，表示未找到
						for (var j = 0; j < Eq_item.length; j++) {
							if (Eq_item[j][0] === itemId) {
								select = j;
								break;  // 找到匹配項，跳出循環
							}
						}
