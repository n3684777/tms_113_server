...
function action(mode,type,selection){
	...
	if(status==0){
            ...
            if (cm.getInventory(1).getItem(i) != null) {
                if (Packages.server.MapleItemInformationProvider.getInstance().isCash(cm.getInventory(1).getItem(i).getItemId()) && cm.getInventory(1).getItem(i).getExpiration()<0) {
					...
					if (可附魔次數 == 0) {
						...
						avail += "#L" + Math.abs(i) + "##i"+cm.getInventory(1).getItem(i).getItemId()+"#  #z" + cm.getInventory(1).getItem(i).getItemId() + "##l \r\n\r\n【#b已附魔#r"+附魔次數+"#k次/#b可附魔#r"+可附魔次數+"#k次】\r\n\r\n";	
					}
                }
            }
        }
        if(avail == ""){
             avail += "\r\n\r\n在您身上並未檢測到有點數裝備，請檢查是否有點數裝備";    
            }
        useGash = cm.getPlayer().getCSPoints(1)>= meso;
        useMaplePoint = cm.getPlayer().getCSPoints(2)>= meso;
        cm.sendSimple((useMaplePoint || useGash) ? "#d選擇第一件裝備(會與第二件交換素質)#k\r\n#rP.S.此功能需要" + meso + "楓點#k #d:#k\r\n#b" + avail : "使用此功能需要" + meso + "楓點");
	}else if(status == 1){
		i = selection;
		var avail2 = "";
        for (this. i2 = 0; i2 < 96; i2++) {
            if (cm.getInventory(1).getItem(i2) != null) {
				...
				if (Packages.server.MapleItemInformationProvider.getInstance().isCash(cm.getInventory(1).getItem(i2).getItemId()) && cm.getInventory(1).getItem(i2).getExpiration()<0) {	
					itemID = cm.getChar().getInventory(MapleInventoryType.EQUIP).getItem(i2).copy();
					讀取後台附魔次數 = WorldConstants.裝備可附魔次數
					附魔次數 = StringTool.getIntFromString(itemID.getOwner());
					可附魔次數 = StringTool.getIntFromString(itemID.getOwnerquantity());
					if (可附魔次數 == 0) {
							avail2 += "#L" + Math.abs(i2) + "##i"+cm.getInventory(1).getItem(i2).getItemId()+"#  #z" + cm.getInventory(1).getItem(i2).getItemId() + "##l \r\n\r\n【#b已附魔#r"+附魔次數+"#k次/#b可附魔#r"+讀取後台附魔次數+"#k次】\r\n\r\n";
						} else {
							avail2 += "#L" + Math.abs(i2) + "##i"+cm.getInventory(1).getItem(i2).getItemId()+"#  #z" + cm.getInventory(1).getItem(i2).getItemId() + "##l \r\n\r\n【#b已附魔#r"+附魔次數+"#k次/#b可附魔#r"+可附魔次數+"#k次】\r\n\r\n";	
						}
					}
		...
		this. 可附1 = item1.getOwnerquantity();
		this. 可附2 = item2.getOwnerquantity();
		var checkj = 0;
		var res = 0;
        ...
        item1.setOwner(Ow2);
		item2.setOwner(Ow1);
		item1.setOwnerquantity(可附2);
		item2.setOwnerquantity(可附1);
		if (useMaplePoint) {
		    cm.getPlayer().modifyCSPoints(2,-meso,true);
		} else if (useGash) {
		    cm.getPlayer().modifyCSPoints(1,-meso,true);
		}
