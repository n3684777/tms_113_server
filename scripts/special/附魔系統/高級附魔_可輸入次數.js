...
var 顯示機率 = 60;
var 實際機率 = 60;
var 力量 = ""+WorldConstants.高級附魔力量最小值+" ~ "+WorldConstants.高級附魔力量最大值+"";
var 敏捷 = ""+WorldConstants.高級附魔敏捷最小值+" ~ "+WorldConstants.高級附魔敏捷最大值+"";
var 智力 = ""+WorldConstants.高級附魔智力最小值+" ~ "+WorldConstants.高級附魔智力最大值+"";
var 幸運 = ""+WorldConstants.高級附魔幸運最小值+" ~ "+WorldConstants.高級附魔幸運最大值+"";
var 物攻 = ""+WorldConstants.高級附魔物攻最小值+" ~ "+WorldConstants.高級附魔物攻最大值+"";
var 魔攻 = ""+WorldConstants.高級附魔魔攻最小值+" ~ "+WorldConstants.高級附魔魔攻最大值+"";
    ...
    if (status == 0) {
       var output = "";
		...
		if (selection <= 0 || selection > 5000) {
			...
			cm.GAMEPLAYERLOG("高級附魔異常", "使用異常軟體修改數據", 0, selection);
			cm.dispose();
			return;
		}
		次數 = selection;
		...
		if(!返回){
			if (附魔次數 < 讀取後台附魔次數) {
				cm.sendYesNo(cm.自動計算機率_高級附魔(selected, item, 次數, 實際機率));
			} else {
				cm.sendOk("發生異常");
				返回 = true;
			}
